  ...
  1   i=3-itsms
       cff=time+0.5D0*dt
        ...
        ierr=nf_fread(Pair(0,0,i), ncidfrc, pairid,
     &                                              sms_rec, r2dvar)
        ierr=nf_fread(QBFC(0,0), ncidfrc, qbfcid,
     &                                              0, r2dvar)
        if (lsusgrd.eq.1) then
          ierr=nf_fread(sustrg(0,0,i), ncidfrc, susid,
     &                                              sms_rec, u2dvar)
        else
        ...
        if (lsvsgrd.eq.1) then
          ierr=nf_fread(svstrg(0,0,i), ncidfrc, svsid,
     &                                              sms_rec, v2dvar)
        else
     ...
     &              'Read surface air pressure for time =', cff
        write(stdout,'(6x,A,1x,A,1x,g12.4,1x,I4)')
     &              'GET_QBFC --',
     &              'Read quadratic bottom friction coef. for time =', 
     &                                                               cff
