     ...
     &                                ,   vname
      ...
      integer*4 nf_noquantize
      parameter (nf_noquantize = 0)
      integer*4 nf_quantize_bitgroom
      ...
      integer*4 nf_def_var_zstandard
      external nf_def_var_zstandard
      integer*4 nf_inq_var_zstandard
      ...
      integer*4 nf_def_var_quantize
      external nf_def_var_quantize
      integer*4 nf_inq_var_quantize
  ...
  10  if (create_new_file) then
     ...
     &         ierr)
        ...
        if (wrthis(indxQBFC)) then
          lvar=lenstr(vname(1,indxQBFC))
          ierr=nf_def_var (ncid, vname(1,indxQBFC)(1:lvar),
     &                              NF_REAL, 3, r2dgrd, hisQBFC)
          lvar=lenstr(vname(2,indxQBFC))
          ierr=nf_put_att_text (ncid, hisQBFC, 'long_name', lvar,
     &                                  vname(2,indxQBFC)(1:lvar))
          lvar=lenstr(vname(3,indxQBFC))
          ierr=nf_put_att_text (ncid, hisQBFC, 'units',     lvar,
     &                                  vname(3,indxQBFC)(1:lvar))
          lvar=lenstr(vname(4,indxQBFC))
          ierr=nf_put_att_text (ncid, hisQBFC, 'field',     lvar,
     &                                  vname(4,indxQBFC)(1:lvar))
          call nf_add_attribute(ncid, hisQBFC, indxQBFC, 5, NF_REAL, 
     &                                                             ierr)
        endif
     ...
     &             rec+1-ierr,'/', total_rec,', restart is assumed.'
        ...
        if (wrthis(indxQBFC)) then
          lvar=lenstr(vname(1,indxQBFC))
          ierr=nf_inq_varid (ncid, vname(1,indxQBFC)(1:lvar), hisQBFC)
          if (ierr .ne. nf_noerr) then
            write(stdout,1) vname(1,indxQBFC)(1:lvar), hisname(1:lstr)
            goto 99
          endif
        endif
        if (wrthis(indxBostr)) then
