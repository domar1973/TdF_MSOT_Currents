     ...
     &                                ,   vname
      ...
      real SSH_Tamp(0:Lm+1+padd_X,0:Mm+1+padd_E,Ntides)
      common /tides_SSH_Tamp/ SSH_Tamp
      real SSH_Tphase(0:Lm+1+padd_X,0:Mm+1+padd_E,Ntides)
      ...
      common /Z0B_VAR/zob
      real cff
      integer*4 i,j, itide, ierr, lstr,lvar, lenstr, nf_fread,
     &        status, varid,ind
      integer*4 nf_byte
      integer*4 nf_int1
      integer*4 nf_char
      ...
      ind=itide
        ...
        status=nf_get_var1_double(ncidfrc,varid,itide,Tperiod(itide))
        if (status.ne.NF_NOERR) then
          write(6,3) 'tide_period',itide,frcname(1:lstr)
          stop
        endif
        Tperiod(itide)=Tperiod(itide)*3600.D0
        status=nf_inq_varid(ncidfrc,'tide_Eamp',varid)
        status=nf_fread(SSH_Tamp(0,0,itide),
     &                              ncidfrc,varid,ind,r2dvar)
        ...
        status=nf_fread(SSH_Tphase(0,0,itide),
     &                              ncidfrc,varid,ind,r2dvar)
        if (status.ne.NF_NOERR) then
